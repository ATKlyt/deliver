...
public class UserInfoController {
     ...
     * @param files 包含头像、身份证等图片的Multipartfile对象数组
     * @param request 封装HTTP请求的对象
     * @return 结果集
     */
    @RequestMapping(value = "/userInfoRegister",method = RequestMethod.POST)
    @ResponseBody
    public Result userInfoRegister(UserInfo userInfo, DriverInfo driverInfo, Area area, MultipartFile[] files, HttpServletRequest request){
        if(userInfo.getUid() == null ||userInfo.getName() == null ||area.getProvince() == null ||area.getCity() == null ||area.getDistrict() == null ||area.getStatus() == null ||userInfo.getIdentityCard() == null ||driverInfo.getCarType() == null ){
        ...
        if(userInfoId > 0){
            ...
            if(USER.equals(role)) {
                ...
                return new Result("获取角色失败，请重新注册","1",null);
            }
        }else{
